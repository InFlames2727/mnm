	...
	async run(msg, args) {
		if(args.cmdOrGrp.isEnabledIn(msg.guild)) {
			return msg.channel.send(
				`<a:VEGAShayirr:522814478610595841> \`${args.cmdOrGrp.name}\` ${args.cmdOrGrp.group ? 'komutu' : 'grubu'} zaten etkin.`
			);
		}
		args.cmdOrGrp.setEnabledIn(msg.guild, true);
		return msg.channel.send(`<a:VEGAScheck:522828534285205505> \`${args.cmdOrGrp.name}\` ${args.cmdOrGrp.group ? 'komutu' : 'grubu'} ektinleştirildi.`);
	}
};
