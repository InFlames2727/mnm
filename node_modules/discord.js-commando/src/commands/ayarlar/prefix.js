...
module.exports = class PrefixCommand extends Command {
	...
	async run(msg, args) {
		...
		if(msg.guild) {
			if (!this.client.isOwner(msg.author)) {
				if(!msg.member.hasPermission('ADMINISTRATOR')) {
					return msg.channel.send("<a:VEGAShayirr:522814478610595841> Sunucuda sadece `Yönetici` izni olanlar ön-ek değiştirebilir.");
				}
			}
		} else if(!this.client.isOwner(msg.author)) {
			return msg.channel.send("<a:VEGAShayirr:522814478610595841> Küresel (global) ön-ek'i sadece bot yapımcı(ları) değiştirebilir.");
		}
		...
		if(lowercase === 'normal') {
			...
			response = `<a:VEGAScheck:522828534285205505> Ön-ek normale döndürüldü. (ön-ek: ${current}).`;
		} else {
			if(msg.guild) msg.guild.commandPrefix = prefix; else this.client.commandPrefix = prefix;
			response = prefix ? `<a:VEGAScheck:522828534285205505> Ön-ek \`${args.prefix}\` yapıldı.` : "Komut ön-ek'i kaldırıldı!";
		}

		msg.channel.send(`<a:VEGAScheck:522828534285205505> ${response} Komutları çalıştırmak için: ${msg.anyUsage('komut')}.`);
		return null;
	}
};
