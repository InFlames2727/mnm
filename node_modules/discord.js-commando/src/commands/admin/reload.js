...
module.exports = class ReloadCommandCommand extends Command {
	...
	async run(msg, args) {
		...
		if(args.cmdOrGrp.group) {
			msg.channel.send(`<a:VEGAScheck:522828534285205505> \`${args.cmdOrGrp.name}\` adlı komut yeniden başlatıldı.`);
		} else {
			msg.channel.send(`<a:VEGAScheck:522828534285205505> \`${args.cmdOrGrp.name}\` grubunun tüm komutları yeniden başlatıldı.`);
		}
		return null;
	}
};
