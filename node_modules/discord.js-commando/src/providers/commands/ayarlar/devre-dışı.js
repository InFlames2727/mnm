	...
	async run(msg, args) {
		if(!args.cmdOrGrp.isEnabledIn(msg.guild)) {
			return msg.chanel.send(
				`<a:VEGAShayirr:522814478610595841> \`${args.cmdOrGrp.name}\` ${args.cmdOrGrp.group ? 'komutu' : 'grubu'} zaten devre dışı.`
			);
		}
		if(args.cmdOrGrp.guarded) {
			return msg.channel.send(
				`<a:VEGAShayirr:522814478610595841> Bunu devre dışı bırakamazsın: \`${args.cmdOrGrp.name}\` ${args.cmdOrGrp.group ? 'komut' : 'grup'}.`
			);
		}
		args.cmdOrGrp.setEnabledIn(msg.guild, false);
		return msg.channel.send(`<a:VEGAScheck:522828534285205505> \`${args.cmdOrGrp.name}\` ${args.cmdOrGrp.group ? 'komutu' : 'grubu'} devre dışı bırakıldı.`);
	}
};
